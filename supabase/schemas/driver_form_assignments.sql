...
CREATE TABLE IF NOT EXISTS driver_form_assignments (
  ...
  partially_completed BOOLEAN DEFAULT false
);
...
COMMENT ON COLUMN driver_form_assignments.reset_period IS 'Period in days after which the form should be reset';
COMMENT ON COLUMN driver_form_assignments.partial_form_data IS 'Partially completed form data stored as JSON';
COMMENT ON COLUMN driver_form_assignments.partially_completed IS 'Whether the form has been partially completed'; 